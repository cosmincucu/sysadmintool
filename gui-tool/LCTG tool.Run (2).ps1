	...
	function Show-StatusNetshPacketCapture
	{
		...
		else
		{
			foreach ($server in $servers)
			{
				...
				{
					$traces = Invoke-Command -ComputerName $activeServers -ScriptBlock { netsh trace status show }
				}
				Load-DataGridView -DataGridView $datagridviewShowPacketCaptures -Item $traces
				$datagridviewShowPacketCaptures.Visible = $true
			}
		}
	}
