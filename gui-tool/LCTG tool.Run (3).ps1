...
# Source File: \\Mac\Google Drive\Projects\tool\LCTG tool.psf
#------------------------------------------------------------------------
...
<#
    ...
    --------------------------------------------------------------------------------
     ...
     Generated on:       6/1/2016 11:12 AM
     Generated by:        
     Organization:        
    --------------------------------------------------------------------------------
    .DESCRIPTION
...
function Call-LCTG_tool_psf {
	...
	$labelNetworkAdapterName = New-Object 'System.Windows.Forms.Label'
	...
	function Alter-LocalAdmin{
		...
		{
			$progress_bar.AppendText("[$(get-date)] You need to select at least one server ! `n")
		}
		else
		{
			[array]$results = $null
	...
	function Copy-Files{
		...
		else
		{
			foreach ($server in $Servers)
			{
				...
				{
					$progress_bar.AppendText("[$(get-date)] $server is not responding!`n")
				}
			}
		}
	}
...
"@ | Out-Null
	...
	function Start-PacketCapture{
		...
		else
		{
			...
			foreach ($server in $servers)
			{
				...
				{
					$TraceName = "trace_$(get-date -Format ddMMyy).etl"
					Invoke-Command -ComputerName $server -ScriptBlock { Get-ChildItem -Path "$($env:windir)\temp\" }
					Invoke-Command -ComputerName $servers -ArgumentList $TraceName, $CaptureSize -ScriptBlock {
						param ($TraceName,
							   $CaptureSize
						)
						New-NetEventSession -Name $TraceName -LocalFilePath "$($env:windir)\temp\$TraceName" -MaxFileSize $CaptureSize
					}
					while ((Get-Job -State Running).count -gt 0)
					{
						$progress_bar.AppendText("[$(get-date)] Creating NetEventSessions  ! `n")
						Start-Sleep -Milliseconds 500
					}
					Invoke-Command -ComputerName $server -ScriptBlock { Add-NetEventProvider -Name "Microsoft-Windows-TCPIP" -SessionName "session1" }
					while ((Get-Job -State Running).count -gt 0)
					{
						$progress_bar.AppendText("[$(get-date)] Creating NetEventproviderss  ! `n")
						Start-Sleep -Milliseconds 500
					}
					Invoke-Command -ComputerName $server -ScriptBlock { Start-NetEventSession -Name "session" }
					while ((Get-Job -State Running).count -gt 0)
					{
						$progress_bar.AppendText("[$(get-date)] Starting NetEventSessions  ! `n")
						Start-Sleep -Milliseconds 500
					}
				}
			}
	...
	function Stop-PacketCapture{
		...
		else
		{
			...
			foreach ($server in $servers)
			{
				...
				{
					Invoke-Command -ComputerName $server -ScriptBlock { netsh.exe trace stop } -AsJob -JobName $server
				}
			}
		}
	}
	function Show-PacketCapture{}
	...
	function Start-NetshPacketCapture{
		...
		else
		{
			...
			foreach ($server in $servers)
			{
				...
				{
					Invoke-Command -ComputerName $server -ArgumentList $CaptureSize,$Adapter -ScriptBlock {
						param ($CaptureSize)
						netsh.exe trace start capture=yes report=no persistent=no tracefile=$($env:windir)\temp\trace_$($env:computername)_$(get-date -Format ssmmHHddMMyy).etl `
								  ...
								  CaptureInterface=$($(Get-NetAdapter -Name $Adapter).InterfaceGuid) `
					} -AsJob
				}
			}
		}
	...
	function Stop-NetshPacketCapture{
		...
		else
		{
			...
			foreach ($server in $servers)
			{
				...
				{
					Invoke-Command -ComputerName $server -ScriptBlock { netsh.exe trace stop } -AsJob
				}
			}
		}
	}
	...
	$buttonStartPacketCapture_Click={
		Start-NetshPacketCapture -servers $ServerList.CheckedItems -CaptureSize $TextboxCaptureSize.Text -Adapter $textboxNetworkAdapterPacketCapture.Text
	}
...
")
	...
	# PacketCapture
	#
	$PacketCapture.Controls.Add($buttonShowPacketCapture)
	...
	$PacketCapture.UseVisualStyleBackColor = $True
	#
	# buttonShowPacketCapture
	#
	$buttonShowPacketCapture.Anchor = 'Top, Right'
	$buttonShowPacketCapture.Location = '508, 38'
	...
	#
	$buttonStopPacketCapture.Anchor = 'Top, Right'
	...
	#
	$buttonStartPacketCapture.Anchor = 'Top, Right'
	...
	$groupbox1.Location = '3, 96'
	$groupbox1.Name = 'groupbox1'
	$groupbox1.RightToLeft = 'No'
	$groupbox1.Size = '564, 59'
	$groupbox1.TabIndex = 13
	$groupbox1.TabStop = $False
	#
	# checkboxTraceConvertToPcap
	#
	$checkboxTraceConvertToPcap.Location = '15, 35'
	...
	#
	$buttonGetPacketCapture.Anchor = 'Top, Right'
	$buttonGetPacketCapture.Location = '505, 17'
	...
	#
	$TextboxCaptureFolder.Location = '125, 10'
	$TextboxCaptureFolder.Name = 'TextboxCaptureFolder'
	$TextboxCaptureFolder.Size = '100, 18'
	$TextboxCaptureFolder.TabIndex = 7
	#
	# labelLocalFolder
	#
	$labelLocalFolder.Location = '15, 13'
	...
	$groupbox2.Controls.Add($labelNetworkAdapterName)
	...
	#
	$labelNetworkAdapterName.Location = '15, 72'
	$labelNetworkAdapterName.Name = 'labelNetworkAdapterName'
	$labelNetworkAdapterName.Size = '104, 23'
	$labelNetworkAdapterName.TabIndex = 16
	$labelNetworkAdapterName.Text = 'Network Adapter Name'
	#
	...
	$textboxFilterPacketCapture1.Size = '100, 18'
	...
	$textboxNetworkAdapterPacketCapture.Size = '100, 18'
	...
	$TextboxCaptureSize.Size = '100, 18'
