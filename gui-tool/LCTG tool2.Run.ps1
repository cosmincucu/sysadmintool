...
<#
    ...
    --------------------------------------------------------------------------------
     ...
     Generated on:       7/14/2016 11:16 AM
     Generated by:        
     Organization:        
    --------------------------------------------------------------------------------
    .DESCRIPTION
...
function Call-LCTG_tool2_psf {
	...
	$buttonFilterName = New-Object 'System.Windows.Forms.Button'
	$textboxFilterName = New-Object 'System.Windows.Forms.TextBox'
	...
	$RemoteExec = New-Object 'System.Windows.Forms.TabPage'
	$richtextbox1 = New-Object 'System.Windows.Forms.RichTextBox'
	$Settings = New-Object 'System.Windows.Forms.TabPage'
	$labelErrorLogPath = New-Object 'System.Windows.Forms.Label'
	$labelLogPath = New-Object 'System.Windows.Forms.Label'
	$labelDBServer = New-Object 'System.Windows.Forms.Label'
	$textbox4 = New-Object 'System.Windows.Forms.TextBox'
	$LogPathTextBox = New-Object 'System.Windows.Forms.TextBox'
	$ErrorLogPathTextbox = New-Object 'System.Windows.Forms.TextBox'
	$DomainLabel = New-Object 'System.Windows.Forms.Label'
	$DomainTextbox = New-Object 'System.Windows.Forms.TextBox'
	$groupbox3 = New-Object 'System.Windows.Forms.GroupBox'
	$labelRevertToNotepad = New-Object 'System.Windows.Forms.Label'
	$labelReplaceDefaultTextEd = New-Object 'System.Windows.Forms.Label'
	$buttonRevertNotepadReplace = New-Object 'System.Windows.Forms.Button'
	$buttonReplaceDefaultNotepad = New-Object 'System.Windows.Forms.Button'
	...
	[string]$DestDB = "Tools"
	$ActiveServersQuery = "SELECT * FROM $DestDB.[dbo].[ActiveServers];"
	$ActiveServersQueryResults = Invoke-Sqlcmd -ServerInstance $DestServer -Query $ActiveServersQuery -Database $DestDB -QueryTimeout 600
	$adServerNames = $ActiveServersQueryResults.ComputerName | Sort-Object -Unique
	...
	function View-Websites{
		...
		if ($ActiveServers.count -gt 0)
		{
			...
			foreach ($server in $ActiveServers)
			{
				Invoke-Command -ComputerName $server -ArgumentList $Filter -ScriptBlock {param($Filter) Get-Website | select id, name, state, physicalpath | Where-Object {$_.name -match $filter } } -AsJob -JobName $server
			}
			while ($(Get-Job -State Running).count -gt 0)
			{
				Start-Sleep -Milliseconds 100
...
"@ | Out-Null
	...
	function Start-NetshPacketCapture{
		...
		{
			Get-Job | Remove-Job -ErrorAction SilentlyContinue
			Invoke-Command -ComputerName $activeServers -ArgumentList $CaptureSize, $Adapter,$filter -ScriptBlock {
				param ($CaptureSize, $Adapter,$filter)
	...
	function Show-NetshPacketCapture{
		...
		[string[]]$ActiveServers = Check-Servers -Servers $Servers
		if ($ActiveServers.count -gt 0)
		{
			$traces = $null
			...
			Load-DataGridView -DataGridView $datagridviewShowPacketCaptures -Item $($traces | select pscomputername, name, creationtime, lastwritetime, @{ Name = "Kbytes"; Expression = { [math]::Round($_.Length / 1Kb) } })
	...
	function Show-StatusNetshPacketCapture{
		...
		if ($activeServers.count -gt 0)
		{
			...
			{
				Invoke-Command -ComputerName $ActiveServer -ScriptBlock { netsh trace show status} -AsJob -JobName $ActiveServer
			}
			while ($(Get-Job -State Running).count -gt 0)
			{
				$progress_bar.AppendText("[$(get-date)] Command(s) running...`n")
			...
			foreach ($ActiveServer in $ActiveServers)
			{
				$JobResult = $($($(Receive-Job -name $ActiveServer) | Out-String).Split(@("Provider:"), [System.StringSplitOptions]::RemoveEmptyEntries)[0] | foreach { $_.split(" ") }).replace(":", "") | ?{ $_ }
				$object = New-Object -TypeName PSCustomObject
				Add-Member -InputObject $object -MemberType NoteProperty -Name "Server" -Value $ActiveServer
				if ([string]$JobResult -match "There is no trace session currently in progress.")
				{
					Add-Member -InputObject $object -MemberType NoteProperty -Name "Status" -Value "Not Running"
				}
				else
				{
					[int]$i = 0
					while ($i -lt 9)
					{
						$object | Add-Member -MemberType NoteProperty -Name $JobResult[$i] -Value $JobResult[$i + 1]
						$i += 2
					}
				}
				$ObjectsArray += $object
	...
	$buttonStartWebsite_Click = {
		for ($i = 0; $i -lt $WebsitesDataGridView.RowCount; $i++)
		{
			if ($($WebsitesDataGridView.rows[$i].cells[0].value) -eq $true)
			{
				...
				$WebsiteName = $WebsitesDataGridView.rows[$i].cells[3].value
	...
	$buttonFilterName_Click = {
		View-Websites -Servers $ServerList.CheckedItems	-Filter $textboxFilterName.text
	}
	...
	$buttonInLoad_Click={
		for ($i = 0; $i -lt $WebsitesDataGridView.RowCount; $i++)
		{
			...
			{
				$computername = $WebsitesDataGridView.rows[$i].cells[1].value
				$WebsitePath = "$($WebsitesDataGridView.rows[$i].cells[6].value)\LB"
				Invoke-Command -ComputerName $computername -ArgumentList $WebsitePath -ScriptBlock {param($WebsitePath) Get-ChildItem -Path $WebsitePath | Rename-Item -NewName "status.txt" }
			}
		}
		View-Websites -Servers $ServerList.CheckedItems
	}
	$buttonOffLoad_Click = {
		for ($i = 0; $i -lt $WebsitesDataGridView.RowCount; $i++)
		{
			if ($($WebsitesDataGridView.rows[$i].cells[0].value) -eq $true)
			{
				$computername = $WebsitesDataGridView.rows[$i].cells[1].value
				$WebsitePath = "$($WebsitesDataGridView.rows[$i].cells[6].value)\LB"
				Invoke-Command -ComputerName $computername -ArgumentList $WebsitePath -ScriptBlock { param ($WebsitePath) Rename-Item -Path "$WebsitePath\status.txt" -NewName "status_$(get-date -Format "MMHHddmmyyyy").txt" }
			}
		}
		View-Websites -Servers $ServerList.CheckedItems
	}
	...
	####################################################################################################################################
	####################################################################################################################################
	# Part to keep the rows selected on the websites tab
	####################################################################################################################################
	$WebsitesDataGridView.Add_CurrentCellDirtyStateChanged({
			param ($Sender,
				$EventArgs)
	...
	$WebsitesDataGridView.Add_CellValueChanged({
			param ($Sender,
				$EventArgs)
	...
	$WebsitesDataGridView.MultiSelect = $true
	$WebsitesDataGridView.SelectionMode = [System.Windows.Forms.DataGridViewSelectionMode]::FullRowSelect
	####################################################################################################################################
	####################################################################################################################################
	# Part to keep the rows selected on the services tab
	####################################################################################################################################
	$ServicesDataGridView.Add_CurrentCellDirtyStateChanged({
			param ($Sender,
				$EventArgs)
			if ($Sender.IsCurrentCellDirty)
			{
	...
	$ServicesDataGridView.Add_CellValueChanged({
			param ($Sender,
				$EventArgs)
			if ($EventArgs.ColumnIndex -eq 0)
			{
	...
	$ServicesDataGridView.MultiSelect = $true
	$ServicesDataGridView.SelectionMode = [System.Windows.Forms.DataGridViewSelectionMode]::FullRowSelect
	####################################################################################################################################
	####################################################################################################################################
	...
	$buttonViewAppPool_Click={
		View-WebsiteAppPool -Servers $ServerList.CheckedItems
	}
	...
	$datagridviewShowPacketCaptures_DataError=[System.Windows.Forms.DataGridViewDataErrorEventHandler]{
	#Event Argument: $_ = [System.Windows.Forms.DataGridViewDataErrorEventArgs]
		#TODO: Place custom script here
		$_.ThrowException = $false
	}
	
	$buttonReplaceDefaultNotepad_Click={
		Replace-Notepad
	}
	
	$buttonRevertNotepadReplace_Click={
		Replace-Notepad -Revert
	}
	...
	$Form_Cleanup_FormClosed=
	{
		...
		try
		{
			...
			$buttonFilterName.remove_Click($buttonFilterName_Click)
			...
			$datagridviewShowPacketCaptures.remove_DataError($datagridviewShowPacketCaptures_DataError)
			...
			$buttonRevertNotepadReplace.remove_Click($buttonRevertNotepadReplace_Click)
			$buttonReplaceDefaultNotepad.remove_Click($buttonReplaceDefaultNotepad_Click)
...
AAA=')
	...
	$MainForm.Text = 'Lowcost System Administrator Tool  v0.5.2.1'
	...
	$SelectAllServers.Location = '663, 12'
	...
	$tabcontrol1.Controls.Add($RemoteExec)
	...
	$usersdatagridview.Size = '635, 292'
	...
	$CopyFilesProgress.Size = '629, 278'
...
")
	...
	# IIS_websites
	#
	$IIS_websites.Controls.Add($buttonFilterName)
	$IIS_websites.Controls.Add($textboxFilterName)
	...
	$IIS_websites.Size = '635, 361'
	$IIS_websites.TabIndex = 3
	$IIS_websites.Text = 'IIS websites'
	#
	# buttonFilterName
	#
	$buttonFilterName.Anchor = 'Top, Right'
	$buttonFilterName.Location = '447, 54'
	$buttonFilterName.Name = 'buttonFilterName'
	$buttonFilterName.Size = '79, 23'
	$buttonFilterName.TabIndex = 44
	$buttonFilterName.Text = 'Filter by Name'
	$buttonFilterName.UseVisualStyleBackColor = $True
	$buttonFilterName.add_Click($buttonFilterName_Click)
	#
	# textboxFilterName
	#
	$textboxFilterName.Anchor = 'Top, Right'
	$textboxFilterName.Location = '532, 56'
	$textboxFilterName.Name = 'textboxFilterName'
	$textboxFilterName.Size = '100, 18'
	$textboxFilterName.TabIndex = 43
	#
	# checkboxIISResetStagger
	#
	$checkboxIISResetStagger.Anchor = 'Top, Right'
	$checkboxIISResetStagger.Location = '570, 26'
	...
	$buttonReleaseconfig.Location = '258, 27'
	...
	$buttonStopAppPool.Location = '173, 27'
	...
	$buttonStartAppPool.Location = '88, 27'
	...
	$buttonViewAppPool.Location = '3, 27'
	...
	$buttonWebConfig.Location = '258, 4'
	...
	$buttonHostsFile.Location = '173, 53'
	...
	$buttonStopWebsite.Location = '173, 4'
	...
	$buttonStartWebsite.Location = '88, 4'
	...
	$buttonViewWebsites.Location = '3, 4'
	...
	$buttonApplyFilter.Location = '480, 3'
	...
	$ServicesFilterTextBox.Location = '535, 4'
	...
	$ServicesDataGridView.Size = '635, 295'
	...
	$datagridviewShowPacketCaptures.Size = '632, 181'
	$datagridviewShowPacketCaptures.TabIndex = 15
	$datagridviewShowPacketCaptures.Visible = $False
	$datagridviewShowPacketCaptures.add_DataError($datagridviewShowPacketCaptures_DataError)
	#
	...
	$ViewConnections.Size = '635, 361'
	$ViewConnections.TabIndex = 7
	$ViewConnections.Text = 'View Connections'
	#
	# RemoteExec
	#
	$RemoteExec.Controls.Add($richtextbox1)
	$RemoteExec.BackColor = 'Control'
	$RemoteExec.Location = '4, 21'
	$RemoteExec.Name = 'RemoteExec'
	$RemoteExec.Size = '635, 361'
	$RemoteExec.TabIndex = 8
	$RemoteExec.Text = 'RemoteExec'
	#
	# richtextbox1
	#
	$richtextbox1.Anchor = 'Top, Bottom, Left, Right'
	$richtextbox1.Location = '15, 69'
	$richtextbox1.Name = 'richtextbox1'
	$richtextbox1.Size = '606, 274'
	$richtextbox1.TabIndex = 0
	$richtextbox1.Text = ''
	#
	# Settings
	#
	$Settings.Controls.Add($labelErrorLogPath)
	$Settings.Controls.Add($labelLogPath)
	$Settings.Controls.Add($labelDBServer)
	$Settings.Controls.Add($textbox4)
	$Settings.Controls.Add($LogPathTextBox)
	$Settings.Controls.Add($ErrorLogPathTextbox)
	$Settings.Controls.Add($DomainLabel)
	$Settings.Controls.Add($DomainTextbox)
	$Settings.Controls.Add($groupbox3)
	$Settings.BackColor = 'Control'
	$Settings.Location = '4, 21'
	$Settings.Name = 'Settings'
	$Settings.Size = '635, 361'
	$Settings.TabIndex = 9
	$Settings.Text = 'Settings'
	#
	# labelErrorLogPath
	#
	$labelErrorLogPath.Location = '15, 38'
	$labelErrorLogPath.Name = 'labelErrorLogPath'
	$labelErrorLogPath.Size = '92, 23'
	$labelErrorLogPath.TabIndex = 7
	$labelErrorLogPath.Text = 'Error Log Path'
	#
	# labelLogPath
	#
	$labelLogPath.Location = '15, 62'
	$labelLogPath.Name = 'labelLogPath'
	$labelLogPath.Size = '92, 23'
	$labelLogPath.TabIndex = 6
	$labelLogPath.Text = 'Log Path'
	#
	# labelDBServer
	#
	$labelDBServer.Location = '15, 86'
	$labelDBServer.Name = 'labelDBServer'
	$labelDBServer.Size = '92, 23'
	$labelDBServer.TabIndex = 5
	$labelDBServer.Text = 'DB server'
	#
	# textbox4
	#
	$textbox4.Location = '113, 83'
	$textbox4.Name = 'textbox4'
	$textbox4.Size = '152, 18'
	$textbox4.TabIndex = 4
	$textbox4.Text = '629145-SYSADMIN.rslc.local'
	#
	# LogPathTextBox
	#
	$LogPathTextBox.Location = '113, 59'
	$LogPathTextBox.Name = 'LogPathTextBox'
	$LogPathTextBox.Size = '152, 18'
	$LogPathTextBox.TabIndex = 3
	$LogPathTextBox.Text = 'C:\log.txt'
	#
	# ErrorLogPathTextbox
	#
	$ErrorLogPathTextbox.Location = '113, 35'
	$ErrorLogPathTextbox.Name = 'ErrorLogPathTextbox'
	$ErrorLogPathTextbox.Size = '152, 18'
	$ErrorLogPathTextbox.TabIndex = 2
	$ErrorLogPathTextbox.Text = 'C:\errors.txt'
	#
	# DomainLabel
	#
	$DomainLabel.Location = '15, 14'
	$DomainLabel.Name = 'DomainLabel'
	$DomainLabel.Size = '92, 23'
	$DomainLabel.TabIndex = 1
	$DomainLabel.Text = 'Destination domain'
	#
	# DomainTextbox
	#
	$DomainTextbox.Location = '113, 11'
	$DomainTextbox.Name = 'DomainTextbox'
	$DomainTextbox.Size = '152, 18'
	$DomainTextbox.TabIndex = 0
	$DomainTextbox.Text = 'rslc.local'
	#
	# groupbox3
	#
	$groupbox3.Controls.Add($labelRevertToNotepad)
	$groupbox3.Controls.Add($labelReplaceDefaultTextEd)
	$groupbox3.Controls.Add($buttonRevertNotepadReplace)
	$groupbox3.Controls.Add($buttonReplaceDefaultNotepad)
	$groupbox3.Location = '409, 7'
	$groupbox3.Name = 'groupbox3'
	$groupbox3.Size = '222, 70'
	$groupbox3.TabIndex = 43
	$groupbox3.TabStop = $False
	$groupbox3.Text = 'Replace Notepad'
	#
	# labelRevertToNotepad
	#
	$labelRevertToNotepad.Location = '6, 40'
	$labelRevertToNotepad.Name = 'labelRevertToNotepad'
	$labelRevertToNotepad.Size = '125, 23'
	$labelRevertToNotepad.TabIndex = 45
	$labelRevertToNotepad.Text = 'Revert to Notepad'
	#
	# labelReplaceDefaultTextEd
	#
	$labelReplaceDefaultTextEd.Location = '6, 13'
	$labelReplaceDefaultTextEd.Name = 'labelReplaceDefaultTextEd'
	$labelReplaceDefaultTextEd.Size = '125, 23'
	$labelReplaceDefaultTextEd.TabIndex = 44
	$labelReplaceDefaultTextEd.Text = 'Replace default text editor'
	#
	# buttonRevertNotepadReplace
	#
	$buttonRevertNotepadReplace.Font = 'Microsoft Sans Serif, 6.75pt'
	$buttonRevertNotepadReplace.Location = '137, 36'
	$buttonRevertNotepadReplace.Name = 'buttonRevertNotepadReplace'
	$buttonRevertNotepadReplace.Size = '79, 21'
	$buttonRevertNotepadReplace.TabIndex = 43
	$buttonRevertNotepadReplace.Text = 'Revert'
	$buttonRevertNotepadReplace.UseVisualStyleBackColor = $True
	$buttonRevertNotepadReplace.add_Click($buttonRevertNotepadReplace_Click)
	#
	# buttonReplaceDefaultNotepad
	#
	$buttonReplaceDefaultNotepad.Font = 'Microsoft Sans Serif, 6.75pt'
	$buttonReplaceDefaultNotepad.Location = '137, 9'
	$buttonReplaceDefaultNotepad.Name = 'buttonReplaceDefaultNotepad'
	$buttonReplaceDefaultNotepad.Size = '79, 21'
	$buttonReplaceDefaultNotepad.TabIndex = 42
	$buttonReplaceDefaultNotepad.Text = 'Replace'
	$buttonReplaceDefaultNotepad.UseVisualStyleBackColor = $True
	$buttonReplaceDefaultNotepad.add_Click($buttonReplaceDefaultNotepad_Click)
	#
	# ServerList
	#
